...
public class JsCallJava {
    ...
    public JsCallJava (String injectedName, Class injectedCls) {
        try {
            ...
            Log.i(TAG, "注入的js字符串是:" + mPreloadInterfaceJS);
        } catch(Exception e){
    ...
    /***
     * 此方法会根据传入的Method构造一个String，构造的这个返回值String是用来作为存放method的map（mMethodsMap）的key
     * 此框架规定所有的提供给js调用的方法的第一个参数必须是WebView
     * 方法的参数对应关系String->_S; 
     * 				int/long/double->_N;
     * 				JSONObject->_O;
     * 				boolean->_B;
     * 				JsCallback->_F;
     * 				其他->_P;
     * 比如传入一个方法toast(WebView view,String msg),则返回值为toast_S
     * 比如传入一个方法showDialog(WebView view,String title,String content,int phone),则返回值为showDialog_S_S_N
     * @param method
     * @return
     */
    private String genJavaMethodSign (Method method) {
        String sign = method.getName();//得到方法名如：toast
        Class[] argsTypes = method.getParameterTypes();//参数类型（如：WebView,String）
        int len = argsTypes.length;
        if (len < 1 || argsTypes[0] != WebView.class) {
            Log.w(TAG, "method(" + sign + ") must use webview to be first parameter, will be pass");
            return null;
        }
        for (int k = 1; k < len; k++) {//忽略第一个参数WebView第一个参数都是WebView
            Class cls = argsTypes[k];
            if (cls == String.class) {//如果第二个参数是String类型
                sign += "_S";//现在方法名sign值为：toast_S
